...
| apiSettings.env.FIFTYONE_DO_LOG_BUCKET_PATH | string | `nil` | Cloud bucket prefix where delegated operation logs are uploaded after the retention period expires, e.g. `gs://my-bucket/do-logs` or `s3://my-bucket/do-logs`. If unset, logs are deleted without being uploaded. |
| apiSettings.env.FIFTYONE_DO_LOG_RETENTION_DAYS | int | `30` | The number of days to retain delegated operation logs in MongoDB before archiving or deleting them. |
