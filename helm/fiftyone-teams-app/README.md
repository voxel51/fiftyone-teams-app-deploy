...
| delegatedOperatorExecutorSettings.env.FIFTYONE_MEDIA_CACHE_SIZE_BYTES | int | `-1` | Set the media cache size (in bytes) for the local FiftyOne Plugins processes. The default value is 32 GiB. `-1` is disabled. |
...
| delegatedOperatorExecutorSettings.replicaCount | int | `0` | Number of pods in the delegated-operator-executor deployment's ReplicaSet. This should not exceed the value set in the deployment's license file for  max concurrent delegated operators. Set to 0 by default, meaning delegated operations will not be executed  without an external executor system. |
...
| secret.fiftyone.delegatedOperatorApiKey | string | `""` | An admin API key to use for running delegated operations, like a service account. <https://docs.voxel51.com/teams/api_connection.html#generating-an-api-key> This key is only required if `.Values.delegatedOperatorExecutorSettings.replicaCount` is nonzero. |
