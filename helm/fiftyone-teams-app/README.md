...
Please see
[configuring delegated operations](https://github.com/voxel51/fiftyone-teams-app-deploy/blob/main/helm/docs/configuring-delegated-operators.md).
...
|-----|------|---------|-------------|
| activitySettings | object | `{"enabled":false,"image":{"pullPolicy":"IfNotPresent","repository":"us-central1-docker.pkg.dev/computer-vision-team/dev-docker/fiftyone-activity","tag":"6ab5164b4bbcd5b277c6a640a9e7e1c3799dfeb6"},"mongo":{"database":""},"orgId":"","redis":{"enabled":true,"image":"redis:7","url":""},"resources":{"limits":{"cpu":"500m","memory":"512Mi"},"requests":{"cpu":"100m","memory":"128Mi"}},"snapshotIntervalMs":"","workers":{"ingest":{"command":["fiftyone-activity-ingest-worker"],"replicaCount":2},"rollup":{"command":["fiftyone-activity-rollup-worker"],"recreate":true,"replicaCount":1},"snapshot":{"command":["fiftyone-activity-snapshot-worker"],"recreate":true,"replicaCount":1}}}` | --------------------------------------------------------------------------- |
| activitySettings.enabled | bool | `false` | Master toggle for the Activity workers + Redis. OFF by default: the fiftyone-activity worker image is not yet published publicly (the main-chart docker-pulls gate), so deployments opt in per env. |
| activitySettings.image.repository | string | `"us-central1-docker.pkg.dev/computer-vision-team/dev-docker/fiftyone-activity"` | Pinned worker image published from the fiftyone-activity repo. |
| activitySettings.image.tag | string | `"6ab5164b4bbcd5b277c6a640a9e7e1c3799dfeb6"` | REQUIRED when enabled: the published image tag (e.g. a commit SHA). 0.0.22: work.judged renamed workflow.decision_attributed (+ rollup decisions_attributed) |
| activitySettings.mongo.database | string | `""` | Database for the activity_* collections. EMPTY (default) co-locates them in the per-deployment FiftyOne database (isolated per instance on a shared Mongo). Set a name only to force a dedicated database. |
| activitySettings.orgId | string | `""` | Override the org the snapshot worker stamps its events with; defaults to appSettings.env.FIFTYONE_ACTIVITY_ORG_ID (rollups are tenant-scoped, so an org-less snapshot would never surface). |
| activitySettings.redis.enabled | bool | `true` | Bundle an ephemeral in-cluster Redis (set false to use an external one). |
| activitySettings.redis.url | string | `""` | Override to point the WORKERS at an external Redis; default targets the bundled one. NOT a global override: the event PRODUCERS (teams-api, fiftyone-app, teams-plugins, delegated operators) each carry their own FIFTYONE_MQ_REDIS_URL env default — override those alongside this or producers keep pointing at the (absent) bundled Redis and their events are silently dropped. |
| activitySettings.snapshotIntervalMs | string | `""` | Snapshot cadence in ms. Defaults to hourly, matching the HOUR bucket the state metrics (dataset_count/sample_count/label_density) roll up by. |
| activitySettings.workers | object | `{"ingest":{"command":["fiftyone-activity-ingest-worker"],"replicaCount":2},"rollup":{"command":["fiftyone-activity-rollup-worker"],"recreate":true,"replicaCount":1},"snapshot":{"command":["fiftyone-activity-snapshot-worker"],"recreate":true,"replicaCount":1}}` | One Deployment per entry; command selects the entrypoint in the image. |
...
| apiSettings.env.FIFTYONE_DO_REQUEUE_DELAY_MINUTES | int | `30` | The number of minutes an operation can sit in a queued state before being requeued. Can be any positive number. |
...
| apiSettings.env.FIFTYONE_MQ_REDIS_URL | string | `"redis://activity-redis:6379/0"` | Activity Analytics: teams-api hosts the domain-event bridge, which emits activity events onto the queue. Without this the bridge catches events but cannot enqueue them, so every bridged event is silently dropped. Points at the bundled in-cluster Redis (same as the workers). |
| apiSettings.env.FIFTYONE_SERVICE_POD_READY_TIMEOUT_S | int | `1800` | The number of seconds to wait for service orchestrator pods to start passing their healthchecks before marking the service with an Error state. |
...
| apiSettings.podSecurityContext | object | `{"fsGroup":1000,"runAsGroup":1000,"runAsNonRoot":true,"runAsUser":1000}` | Pod-level security attributes and common container settings for `teams-api`. [Reference][security-context]. UID/GID 1000 matches the image; set explicitly to satisfy PSA `restricted`. |
...
| appSettings.env.FIFTYONE_ACTIVITY_ORG_ID | string | `"dev-internal-env"` | The org id the activity feed is scoped by (matches user.organization.id). Single-org deployments set this so the review-submit emit stamps the org the reader queries. REPLACE with your org id. |
...
| appSettings.env.FIFTYONE_MQ_REDIS_URL | string | `"redis://activity-redis:6379/0"` | Activity Analytics: where the workflows-plugin emit() enqueues events (the bundled in-cluster Redis). Emit is fire-and-forget; if unset the review_submitted events simply don't flow. |
...
| appSettings.podSecurityContext | object | `{"fsGroup":1000,"runAsGroup":1000,"runAsNonRoot":true,"runAsUser":1000}` | Pod-level security attributes and common container settings for `fiftyone-app`. [Reference][security-context]. UID/GID 1000 matches the image; set explicitly to satisfy PSA `restricted`. |
...
| appSettings.resources | object | `{"limits":{"cpu":2,"ephemeral-storage":"8Gi","memory":"6Gi"},"requests":{"cpu":"500m","ephemeral-storage":"8Gi","memory":"512Mi"}}` | Container resource requests and limits for `fiftyone-app`. [Reference][resources]. ephemeral-storage is set explicitly: the fiftyone-app-torch image is ~5.4GB compressed / ~15-20GB unpacked, but GKE Autopilot injects a 1Gi default when unset and bin-packs by request — so the app landed on a full node and was evicted ("node was low on resource: ephemeral-storage"). Autopilot caps the POD SUM at 10Gi (warden rejects above it, counting every container), so leave headroom for injected sidecars — the ephem envs add a 1Gi telemetry sidecar, and 10Gi here made the pod 11Gi and unschedulable (Argo sync bounced silently for days). 8Gi + sidecars fits. Harmless on non-Autopilot. |
...
| delegatedOperatorDeployments.template | object | `{"affinity":{},"deploymentAnnotations":{},"description":"","env":{"FIFTYONE_ACTIVITY_ORG_ID":"dev-internal-env","FIFTYONE_DELEGATED_OPERATION_LOG_PATH":"","FIFTYONE_INTERNAL_SERVICE":true,"FIFTYONE_MEDIA_CACHE_SIZE_BYTES":-1,"FIFTYONE_MQ_REDIS_URL":"redis://activity-redis:6379/0"},"image":{"pullPolicy":"Always","repository":"voxel51/fiftyone-teams-cv-full","tag":""},"labels":{},"liveness":{"failureThreshold":5,"periodSeconds":30,"timeoutSeconds":30},"nodeSelector":{},"podAnnotations":{},"podDisruptionBudget":{"enabled":false,"minAvailable":null},"podSecurityContext":{"fsGroup":1000,"runAsGroup":1000,"runAsNonRoot":true,"runAsUser":1000},"readiness":{"failureThreshold":5,"periodSeconds":30,"timeoutSeconds":30},"replicaCount":3,"resources":{"limits":{},"requests":{}},"secretEnv":{},"securityContext":{},"startup":{"failureThreshold":5,"periodSeconds":30,"timeoutSeconds":30},"tolerations":[],"topologySpreadConstraints":[],"updateStrategy":{"type":"RollingUpdate"},"volumeMounts":[],"volumes":[]}` | A common template applied to all deployments. Each deployment can then override individual fields as needed by the operator. |
...
| delegatedOperatorDeployments.template.env.FIFTYONE_MQ_REDIS_URL | string | `"redis://activity-redis:6379/0"` | Activity Analytics: delegated operators (incl. workflow stage ops) run here, so their emit() needs the queue. Without the Redis URL, fiftyone.mq defaults to localhost:6379 (unreachable) and events are silently dropped. Same in-cluster Redis + org id as the other services. |
...
| delegatedOperatorDeployments.template.podSecurityContext | object | `{"fsGroup":1000,"runAsGroup":1000,"runAsNonRoot":true,"runAsUser":1000}` | Pod-level security attributes and common container settings for `delegated-operator-executor`. [Reference][security-context]. UID/GID 1000 matches the image; set explicitly to satisfy PSA `restricted`. |
...
| delegatedOperatorJobTemplates.serviceOrchestrators | object | two chart-managed orchestrators (see values.yaml) | On-Demand Service Orchestrators for the `kubernetes-service` environment (long-lived push services). Each entry renders a jinja2 `Pod` manifest into the same `ConfigMap` as `jobs`, merged over `template` with the same logic (job-only fields do not apply). Entries also accept `description`, `registerOrchestrator`, and a `services` map of the service definitions the orchestrator runs, whose identity fields are derived from the map keys. @schema type: object properties: {} @schema |
| delegatedOperatorJobTemplates.template | object | `{"activeDeadlineSeconds":null,"affinity":{},"backoffLimit":null,"completions":null,"containerSecurityContext":{},"env":{"FIFTYONE_DELEGATED_OPERATION_LOG_PATH":"","FIFTYONE_MEDIA_CACHE_SIZE_BYTES":-1},"image":{"pullPolicy":"Always","repository":"voxel51/fiftyone-teams-cv-full","tag":""},"jobAnnotations":{},"labels":{},"nodeSelector":{},"podAnnotations":{},"podSecurityContext":{"fsGroup":1000,"runAsGroup":1000,"runAsNonRoot":true,"runAsUser":1000},"registerOrchestrator":true,"resources":{"limits":{},"requests":{}},"secretEnv":{},"tolerations":[],"ttlSecondsAfterFinished":null,"volumeMounts":[],"volumes":[]}` | A common template applied to all deployments. Each deployment can then override individual fields as needed by the operator. |
...
| delegatedOperatorJobTemplates.template.backoffLimit | optional | `nil` | Amount of retries for the job to try before being marked as "Failed". [Reference][job-backoff-failure-policy]. |
| delegatedOperatorJobTemplates.template.completions | optional | `nil` | Number of completions to run before marking the job as "Completed". [Reference][job-termination-and-cleanup]. |
| delegatedOperatorJobTemplates.template.containerSecurityContext | object | `{}` | Container security configuration for generated delegated-operator containers. [Reference][container-security-context]. |
| delegatedOperatorJobTemplates.template.env.FIFTYONE_DELEGATED_OPERATION_LOG_PATH | string | `""` | Full path to a network-mounted file system or a cloud storage path to use for storing logs generated by delegated operation runs, one file per job. The default `""` means log upload is disabled. |
| delegatedOperatorJobTemplates.template.env.FIFTYONE_MEDIA_CACHE_SIZE_BYTES | int | `-1` | Set the media cache size (in bytes) for the local FiftyOne Delegated Operator Executor processes. When not set, the app's default value is 32 GiB. `-1` disables the media cache garbage collection (and cache size is unlimited). |
| delegatedOperatorJobTemplates.template.image.pullPolicy | string | `"Always"` | Instruct when the kubelet should pull (download) the specified image. One of `IfNotPresent`, `Always` or `Never`. [Reference][image-pull-policy]. |
| delegatedOperatorJobTemplates.template.image.repository | string | `"voxel51/fiftyone-teams-cv-full"` | Container image for `delegated-operator-executor`. |
...
| delegatedOperatorJobTemplates.template.podSecurityContext | object | `{"fsGroup":1000,"runAsGroup":1000,"runAsNonRoot":true,"runAsUser":1000}` | Pod-level security attributes and common container settings for `delegated-operator-executor`. [Reference][security-context]. UID/GID 1000 matches the image; set explicitly to satisfy PSA `restricted`. |
| delegatedOperatorJobTemplates.template.registerOrchestrator | bool | `true` | Controls whether a post-install/post-upgrade hook seeding Job registers an orchestrator in MongoDB for each entry under `jobs` and `services` Individual entries may override this with their own `registerOrchestrator` key. See `./templates/seed-orchestrators-job.yaml` and `./files/seed_orchestrators.py`. |
| delegatedOperatorJobTemplates.template.resources | object | `{"limits":{},"requests":{}}` | Container resource requests and limits for `delegated-operator-executor`. [Reference][resources]. |
| delegatedOperatorJobTemplates.template.secretEnv | object | `{}` | Secret variables to be passed to the delegated-operator-executor containers. |
| delegatedOperatorJobTemplates.template.tolerations | list | `[]` | Allow the k8s scheduler to schedule delegated-operator-executor pods with matching taints. [Reference][taints-and-tolerations]. |
| delegatedOperatorJobTemplates.template.ttlSecondsAfterFinished | optional | `nil` | Number of seconds for a job to sit idle before being automatically removed. [Reference][job-termination-and-cleanup]. |
...
| pluginsSettings.env.FIFTYONE_ACTIVITY_ORG_ID | string | `"dev-internal-env"` |  |
...
| pluginsSettings.env.FIFTYONE_MQ_REDIS_URL | string | `"redis://activity-redis:6379/0"` | Activity Analytics: the workflows plugin (annotate/review submit) runs in THIS pod, so its emit() must reach the queue here. Without the Redis URL, fiftyone.mq falls back to localhost:6379 (unreachable) and every annotation/review event is silently dropped. Points at the bundled in-cluster Redis; the org id matches user.organization.id (rollups are tenant-scoped, so a wrong/missing org id makes the events invisible). |
...
| pluginsSettings.podSecurityContext | object | `{"fsGroup":1000,"runAsGroup":1000,"runAsNonRoot":true,"runAsUser":1000}` | Pod-level security attributes and common container settings for `teams-plugins`. [Reference][security-context]. UID/GID 1000 matches the image; set explicitly to satisfy PSA `restricted`. |
...
| teamsAppSettings.env.FIFTYONE_APP_TEAMS_SDK_RECOMMENDED_VERSION | string | `"2.23.0"` | The recommended fiftyone SDK version that will be displayed in the install modal (i.e. `pip install ... fiftyone==0.11.0`). |
...
| teamsAppSettings.env.VFF_WF_ACTIVITY | string | `"true"` | TEMPORARY dev flag: shows the workflow Activity tab (raw event tail, manager-only) for verifying event capture. Remove with the tab. |
| teamsAppSettings.env.VFF_WF_METRIC | string | `"true"` | Dev flag: shows the workflow Metrics tab (FOEPD-4342 pre-release gate). |
...
[job-backoff-failure-policy]: https://kubernetes.io/docs/concepts/workloads/controllers/job/#pod-backoff-failure-policy
