...
| activitySettings.orgId | string | `""` | Organization the snapshot worker stamps on its events. Defaults to `appSettings.env.FIFTYONE_ACTIVITY_ORG_ID`. Rollups are scoped per organization, so an unset value hides the snapshot metrics. |
