...
|-----|------|---------|-------------|
| activitySettings | object | `{"enabled":true,"image":{"pullPolicy":"IfNotPresent","repository":"us-central1-docker.pkg.dev/computer-vision-team/dev-docker/fiftyone-activity","tag":"5613c29aee74653e893f0dd27eb717e5f4235f37"},"mongo":{"database":""},"orgId":"","redis":{"enabled":true,"image":"redis:7","url":""},"resources":{"limits":{"cpu":"500m","memory":"512Mi"},"requests":{"cpu":"100m","memory":"128Mi"}},"snapshotIntervalMs":"","workers":{"ingest":{"command":["fiftyone-activity-ingest-worker"],"replicaCount":2},"rollup":{"command":["fiftyone-activity-rollup-worker"],"recreate":true,"replicaCount":1},"snapshot":{"command":["fiftyone-activity-snapshot-worker"],"recreate":true,"replicaCount":1}}}` | --------------------------------------------------------------------------- |
...
| activitySettings.image.tag | string | `"5613c29aee74653e893f0dd27eb717e5f4235f37"` | REQUIRED when enabled: the published image tag (e.g. a commit SHA). 0.0.21: allowDiskUse on aggregate reads; aclose loop teardown |
...
| appSettings.env.SKIP_OPERATOR_PERMISSION_CHECK | string | `"true"` | DEV ENV ONLY: bypass the teams-api builtin-operator allowlist. That list is enumerated from the fiftyone.yml bundled in teams-api's OWN SDK install, so newly added plugin operators 403 until the hub image ships an SDK containing them. Remove once the hub pin includes the declared activity operators. |
...
| appSettings.resources | object | `{"limits":{"cpu":2,"ephemeral-storage":"10Gi","memory":"6Gi"},"requests":{"cpu":"500m","ephemeral-storage":"10Gi","memory":"512Mi"}}` | Container resource requests and limits for `fiftyone-app`. [Reference][resources]. ephemeral-storage is set explicitly: the fiftyone-app-torch image is ~5.4GB compressed / ~15-20GB unpacked, but GKE Autopilot injects a 1Gi default when unset and bin-packs by request — so the app landed on a full node and was evicted ("node was low on resource: ephemeral-storage"). 10Gi is Autopilot's per-workload max (it rejects >10Gi). Harmless on non-Autopilot clusters. |
