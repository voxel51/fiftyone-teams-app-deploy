...
1. All pods, in the environment (`*.env`):
    ...
    > [dedicated `teams-plugins`](#fiftyone-teams-plugins) deployment
    > you will need to include `teams-plugins` in your `NO_PROXY` and `no_proxy`
    > configurations
...
1. The pod based on the `fiftyone-teams-app` image (`teamsAppSettings.env`)
    ...
    > **NOTE**: If you have enabled a
    > [dedicated `teams-plugins`](#fiftyone-teams-plugins) deployment
    > you will need to include `teams-plugins` in your `GLOBAL_AGENT_NO_PROXY``
    > configuration
