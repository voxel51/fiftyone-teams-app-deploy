...
Prior to v2.2.1, inviting users to join your FiftyOne Teams instance was not supported
when `FIFTYONE_AUTH_MODE` is set to `internal`.
Starting in v2.2.1+, you can enable invitations for your organization through the
...
| teamsAppSettings.env.FIFTYONE_APP_TEAMS_SDK_RECOMMENDED_VERSION | string | `"2.2.1"` | The recommended fiftyone SDK version that will be displayed in the install modal (i.e. `pip install ... fiftyone==0.11.0`). |
...
> **NOTE**: Upgrading to FiftyOne Teams v2.2.1 _requires_
...
> **NOTE**: Upgrading to FiftyOne Teams v2.2.1 _requires_ a license file.
...
1. [Upgrade to FiftyOne Teams v2.2.1](#deploying-fiftyone-teams)
    > **NOTE**: At this step, FiftyOne SDK users will lose access to the
    > FiftyOne Teams Database until they upgrade to `fiftyone==2.2.1`
1. Upgrade your FiftyOne SDKs to version 2.2.1
...
1. In your `values.yaml`, set the required values
    ...
    1. `secret.fiftyone.fiftyoneAuthSecret` (or your deployment's equivalent)
        1. This sets the `FIFTYONE_AUTH_SECRET` environment variable
           in the appropriate service pods
1. [Upgrade to FiftyOne Teams version 2.2.1](#deploying-fiftyone-teams)
1. Upgrade FiftyOne Teams SDK users to FiftyOne Teams version 2.2.1
    - Login to the FiftyOne Teams UI
...
1. Upgrade all the datasets
    ...
    > **NOTE** Any FiftyOne SDK less than 2.2.1 will lose connectivity after
    > this point.
    > Upgrading all SDKs to `fiftyone==2.2.1` is recommended before migrating
        > your database.
...
1. Voxel51 recommends upgrading all FiftyOne Teams SDK users to FiftyOne Teams
   version 2.2.1, but it is not required
    - Login to the FiftyOne Teams UI
    - To obtain the CLI command to install the FiftyOne SDK associated with
      your FiftyOne Teams version, navigate to `Account > Install FiftyOne`
