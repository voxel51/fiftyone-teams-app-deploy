...
| delegatedOperatorJobTemplates.rbac.serviceAccount.annotations | object | `{}` | `ServiceAcount` annotations. [Reference][annotations]. |
| delegatedOperatorJobTemplates.rbac.serviceAccount.create | bool | `true` | Controls whether to create the `ServiceAcount` named `delegatedOperatorJobTemplates.rbac.serviceAccount.name`. |
| delegatedOperatorJobTemplates.rbac.serviceAccount.labels | object | `{}` | Additional labels for the generated `ServiceAcount`. [Reference][labels-and-selectors]. |
| delegatedOperatorJobTemplates.rbac.serviceAccount.name | string | `""` | Name of the `ServiceAcount` (existing or to be created) in the namespace `namespace.name` used for DO management. Defaults to `release-name-fiftyone-teams-app-do-management`. |
...
| teamsAppSettings.env.FIFTYONE_APP_TEAMS_SDK_RECOMMENDED_VERSION | string | `"2.14.0"` | The recommended fiftyone SDK version that will be displayed in the install modal (i.e. `pip install ... fiftyone==0.11.0`). |
