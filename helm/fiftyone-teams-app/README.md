...
| teamsAppSettings.env.FIFTYONE_APP_TEAMS_SDK_RECOMMENDED_VERSION | string | `"0.15.9"` | The recommended fiftyone SDK version that will be displayed in the install modal (i.e. `pip install ... fiftyone==0.11.0`). |
...
The FiftyOne 0.15.9 SDK (database version 0.23.8) is _NOT_ backwards-compatible
...
1. [Upgrade to FiftyOne Teams version 1.5.9](#launch-fiftyone-teams)
   ...
   (this is not the default value in `values.yaml` and must be overridden).
    ...
    > FiftyOne Teams Database until they upgrade to `fiftyone==0.15.9`
1. Upgrade your FiftyOne SDKs to version 0.15.9
...
1. Have an admin run to upgrade all datasets
    ...
    > **NOTE** Any FiftyOne SDK less than 0.15.9 will lose database connectivity
    >  at this point. Upgrading to `fiftyone==0.15.9` is required
