...
| delegatedOperatorJobTemplates.template.registerOrchestrator | bool | `true` | Controls whether a post-install/post-upgrade seeding Job registers an orchestrator in MongoDB for each entry under `jobs` and `services` (upserted by `instance_id` = entry name). Individual entries can override this with their own `registerOrchestrator` key. See templates/seed-orchestrators-job.yaml and files/seed_orchestrators.py. |
