...
| delegatedOperatorInstances.teamsDo | object | `{"affinity":{},"env":{"FIFTYONE_DELEGATED_OPERATION_LOG_PATH":"","FIFTYONE_INTERNAL_SERVICE":true,"FIFTYONE_MEDIA_CACHE_SIZE_BYTES":-1},"image":{"pullPolicy":"Always","repository":"voxel51/fiftyone-teams-cv-full","tag":""},"labels":{},"liveness":{"failureThreshold":5,"periodSeconds":30,"timeoutSeconds":30},"name":"teams-do","nodeSelector":{},"podAnnotations":{},"podSecurityContext":{},"readiness":{"failureThreshold":5,"periodSeconds":30,"timeoutSeconds":30},"replicaCount":3,"resources":{"limits":{},"requests":{}},"secretEnv":{},"securityContext":{},"startup":{"failureThreshold":5,"periodSeconds":30,"timeoutSeconds":30},"tolerations":[],"volumeMounts":[],"volumes":[]}` | Multiple instances can be configured. A default configuration is provided below. The name of the deployment will be the map key in kebabcase. |
| delegatedOperatorInstances.teamsDo.affinity | object | `{}` | Affinity and anti-affinity for delegated-operator-executor. [Reference][affinity]. |
| delegatedOperatorInstances.teamsDo.env.FIFTYONE_DELEGATED_OPERATION_LOG_PATH | string | `""` | Full path to a network-mounted file system or a cloud storage path to use for storing logs generated by delegated operation runs, one file per job. The default `""` means log upload is disabled. |
| delegatedOperatorInstances.teamsDo.env.FIFTYONE_INTERNAL_SERVICE | bool | `true` | Whether the SDK is running in an internal service context. When running in FiftyOne Teams, set to `true`. |
| delegatedOperatorInstances.teamsDo.env.FIFTYONE_MEDIA_CACHE_SIZE_BYTES | int | `-1` | Set the media cache size (in bytes) for the local FiftyOne Delegated Operator Executor processes. The default value is 32 GiB. `-1` is disabled. |
| delegatedOperatorInstances.teamsDo.image.pullPolicy | string | `"Always"` | Instruct when the kubelet should pull (download) the specified image. One of `IfNotPresent`, `Always` or `Never`. [Reference][image-pull-policy]. |
| delegatedOperatorInstances.teamsDo.image.repository | string | `"voxel51/fiftyone-teams-cv-full"` | Container image for delegated-operator-executor. |
| delegatedOperatorInstances.teamsDo.image.tag | string | `""` | Image tag for delegated-operator-executor. Defaults to the chart version. |
| delegatedOperatorInstances.teamsDo.labels | object | `{}` | Additional labels for the `delegated-operator-executor` deployment. [Reference][labels-and-selectors]. |
| delegatedOperatorInstances.teamsDo.liveness.failureThreshold | int | `5` | Number of times to retry the liveness probe for the teams-do. [Reference][probes]. |
| delegatedOperatorInstances.teamsDo.liveness.periodSeconds | int | `30` | How often (in seconds) to perform the liveness probe for teams-do. [Reference][probes]. |
| delegatedOperatorInstances.teamsDo.liveness.timeoutSeconds | int | `30` | Timeout for the liveness probe for the teams-do. [Reference][probes]. |
| delegatedOperatorInstances.teamsDo.name | string | `"teams-do"` | Deployment name |
| delegatedOperatorInstances.teamsDo.nodeSelector | object | `{}` | nodeSelector for delegated-operator-executor. [Reference][node-selector]. |
| delegatedOperatorInstances.teamsDo.podAnnotations | object | `{}` | Annotations for delegated-operator-executor pods. [Reference][annotations]. |
| delegatedOperatorInstances.teamsDo.podSecurityContext | object | `{}` | Pod-level security attributes and common container settings for delegated-operator-executor. [Reference][security-context]. |
| delegatedOperatorInstances.teamsDo.readiness | object | `{"failureThreshold":5,"periodSeconds":30,"timeoutSeconds":30}` | Container security configuration for delegated-operator-executor. [Reference][container-security-context]. |
| delegatedOperatorInstances.teamsDo.readiness.failureThreshold | int | `5` | Number of times to retry the readiness probe for the teams-do. [Reference][probes]. |
| delegatedOperatorInstances.teamsDo.readiness.periodSeconds | int | `30` | How often (in seconds) to perform the readiness probe for teams-do. [Reference][probes]. |
| delegatedOperatorInstances.teamsDo.readiness.timeoutSeconds | int | `30` | Timeout for the readiness probe for the teams-do. [Reference][probes]. |
| delegatedOperatorInstances.teamsDo.replicaCount | int | `3` | Number of pods in the delegated-operator-executor deployment's ReplicaSet. This should not exceed the value set in the deployment's license file for  max concurrent delegated operators, which defaults to 3. |
| delegatedOperatorInstances.teamsDo.resources | object | `{"limits":{},"requests":{}}` | Container resource requests and limits for delegated-operator-executor. [Reference][resources]. |
| delegatedOperatorInstances.teamsDo.secretEnv | object | `{}` | Secret variables to be passed to the delegated-operator-executor containers. |
| delegatedOperatorInstances.teamsDo.startup.failureThreshold | int | `5` | Number of times to retry the startup probe for the teams-do. [Reference][probes]. |
| delegatedOperatorInstances.teamsDo.startup.periodSeconds | int | `30` | How often (in seconds) to perform the startup probe for teams-do. [Reference][probes]. |
| delegatedOperatorInstances.teamsDo.startup.timeoutSeconds | int | `30` | Timeout for the startup probe for the teams-do. [Reference][probes]. |
| delegatedOperatorInstances.teamsDo.tolerations | list | `[]` | Allow the k8s scheduler to schedule delegated-operator-executor pods with matching taints. [Reference][taints-and-tolerations]. |
| delegatedOperatorInstances.teamsDo.volumeMounts | list | `[]` | Volume mounts for delegated-operator-executor pods. [Reference][volumes]. |
| delegatedOperatorInstances.teamsDo.volumes | list | `[]` | Volumes for delegated-operator-executor. [Reference][volumes]. |
