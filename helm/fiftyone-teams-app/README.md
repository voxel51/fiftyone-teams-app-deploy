...
| teamsAppSettings.env.FIFTYONE_APP_ANONYMOUS_ANALYTICS_ENABLED | bool | `true` | Controls whether anonymous analytics are captures for the teams application. Set to false to opt-out of anonymous analytics. |
| teamsAppSettings.env.FIFTYONE_APP_TEAMS_SDK_RECOMMENDED_VERSION | string | `"2.0.1"` | The recommended fiftyone SDK version that will be displayed in the install modal (i.e. `pip install ... fiftyone==0.11.0`). |
...
1. [Upgrade to FiftyOne Teams v2.0.1](#deploying-fiftyone-teams)
    ...
    > FiftyOne Teams Database until they upgrade to `fiftyone==2.0.1`
...
1. Upgrade all the datasets
    ...
    > **NOTE** Any FiftyOne SDK less than 2.0.1 will lose connectivity after
    > this point.
    > Upgrading all SDKs to `fiftyone==2.0.1` is recommended before migrating
        > your database.
