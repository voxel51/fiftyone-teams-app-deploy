...
| delegatedOperatorJobTemplates.services | object | `{}` | On-Demand Service Orchestrator Pod templates for the `kubernetes-service` environment (long-lived push services). Each entry renders a jinja2 `Pod` manifest into the same `ConfigMap` as `jobs`, merged over `template` with the same logic. Job-only fields (`image`, `jobAnnotations`, and the Job spec fields) do not apply. The kubernetes-service broker provides the image and command per task. Entries also accept `description` and `registerOrchestrator`. |
...
| serviceOrchestrator.enabled | bool | `false` | Controls whether to create the builtin services overrides `ConfigMap` below and volume mount it into `teams-api` container. |
