...
| `FIFTYONE_DO_LOG_BUCKET_PATH`                | Cloud bucket prefix where delegated operation logs are uploaded after the retention period expires, e.g. `gs://my-bucket/do-logs` or `s3://my-bucket/do-logs`. If unset, logs are deleted without being uploaded. The default is `None` (log upload disabled).                 | No                        |
| `FIFTYONE_DO_LOG_RETENTION_DAYS`             | The number of days to retain delegated operation logs in MongoDB before archiving or deleting them. Default is `30`.                                                                                                                                                           | No                        |
