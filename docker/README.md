...
`FIFTYONE_DATABASE_ADMIN` is set to `false` by default for FiftyOne Teams version 1.3.5. This is because FiftyOne Teams version 1.3.5 is backwards compatible with FiftyOne Teams database schema version 0.19 (Teams Version 1.1) and newer.
...
services:
  fiftyone-app:
    image: voxel51/fiftyone-app-torch:v1.3.5
```
...
1. [Upgrade to FiftyOne Teams version 1.3.5](#deploying-fiftyone-teams) with `FIFTYONE_DATABASE_ADMIN=true` (this is not the default in the `config.yaml` for this release).<br>
   **NOTE:** FiftyOne SDK users will lose access to the FiftyOne Teams Database at this step until they upgrade to `fiftyone==0.13.5`
1. Upgrade your FiftyOne SDKs to version 0.13.5<br>
...
1. Use `fiftyone migrate --info` to make sure that all datasets have been migrated to version 0.21.5.
...
The FiftyOne 0.13.5 SDK (database version 0.21.5) is backwards-compatible with FiftyOne Teams Database Versions 0.19.0 and later, but the FiftyOne 0.11.0 SDK is _NOT_ forward compatible with FiftyOne Teams Database Version 0.21.5.
...
1. [Upgrade to FiftyOne Teams version 1.3.5](#deploying-fiftyone-teams)
1. Upgrade FiftyOne Teams SDK users to FiftyOne Teams version 0.13.5<br>
   The command line for installing the FiftyOne SDK associated with your FiftyOne Teams version is available in the FiftyOne Teams UI under `Account > Install FiftyOne` after a user has logged in.
1. Have the admin run `FIFTYONE_DATABASE_ADMIN=true fiftyone migrate --all` to upgrade all datasets<br>
   **NOTE** Any FiftyOne SDK less than 0.13.5 will lose database connectivity at this point; upgrading to `fiftyone==0.13.5` is required
1. Use `fiftyone migrate --info` to ensure that all datasets are now at version 0.21.5.
