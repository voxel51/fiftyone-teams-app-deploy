...
func (s *doK8sConfigMapTemplateTest) TestNativeSidecarTelemetryAutoInject() {
	testCases := []struct {
	...
	}{
		{
			...
			map[string]string{
				"delegatedOperatorJobTemplates.jobs.testJob.unused":                               "nil",
				"delegatedOperatorJobTemplates.template.nativeSidecarContainers[0].name":          "telemetry-sidecar",
			...
			func(subT gruntworkTesting.TestingT, job batchv1.Job) {
				...
				s.Require().NotNil(podSpec.ShareProcessNamespace, "shareProcessNamespace must be set when nativeSidecarContainers present")
				s.True(*podSpec.ShareProcessNamespace, "shareProcessNamespace must be true")

				s.Require().Len(podSpec.InitContainers, 1, "Should be one initContainer (the sidecar)")
				sidecar := podSpec.InitContainers[0]
				s.True(envContains(sidecar.Env, "TARGET_CONTAINER", "executor"), "Sidecar must have auto-injected TARGET_CONTAINER=executor")
				s.True(volumeMountContains(sidecar.VolumeMounts, "telemetry-socket", "/tmp/telemetry"), "Sidecar must have auto-injected telemetry-socket volumeMount")

				executor := findContainer(podSpec.Containers, "executor")
				s.Require().NotNil(executor, "executor container must exist")
				s.True(volumeMountContains(executor.VolumeMounts, "telemetry-socket", "/tmp/telemetry"), "Executor must have auto-injected telemetry-socket volumeMount")

				s.True(volumeContains(podSpec.Volumes, "telemetry-socket"), "Pod must have auto-injected telemetry-socket emptyDir volume")
			},
		},
		{
			"PreservesExplicitTargetContainerOverride",
			map[string]string{
				"delegatedOperatorJobTemplates.jobs.testJob.unused":                               "nil",
				"delegatedOperatorJobTemplates.template.nativeSidecarContainers[0].name":          "telemetry-sidecar",
				"delegatedOperatorJobTemplates.template.nativeSidecarContainers[0].image":         "telemetry-sidecar:latest",
			...
			func(subT gruntworkTesting.TestingT, job batchv1.Job) {
				...
				s.True(envContains(sidecar.Env, "TARGET_CONTAINER", "custom-executor"), "User-set TARGET_CONTAINER must win")
				s.False(envContains(sidecar.Env, "TARGET_CONTAINER", "executor"), "Auto-inject must not also append TARGET_CONTAINER=executor")
			},
		},
		{
			"DoesNotInjectTelemetryWiringForNonExecutorSidecar",
			map[string]string{
				"delegatedOperatorJobTemplates.jobs.testJob.unused":                               "nil",
				"delegatedOperatorJobTemplates.template.nativeSidecarContainers[0].name":          "istio-proxy",
			...
			func(subT gruntworkTesting.TestingT, job batchv1.Job) {
				...
				sidecar := podSpec.InitContainers[0]
				s.False(volumeMountContains(sidecar.VolumeMounts, "telemetry-socket", "/tmp/telemetry"), "Non-executor sidecar must not auto-inject volumeMount")
				executor := findContainer(podSpec.Containers, "executor")
				s.Require().NotNil(executor)
				s.False(volumeMountContains(executor.VolumeMounts, "telemetry-socket", "/tmp/telemetry"), "Executor must not get telemetry-socket mount when no executor sidecar")
			},
		},
	}
	...
	for _, testCase := range testCases {
		...
		s.Run(testCase.name, func() {
			...
			s.Require().NotEmpty(configMap.Data["testJob.yaml"], "testJob.yaml should be rendered in configmap data")

			jinjaArgs := map[string]interface{}{
				"_id":      strings.ToLower(random.UniqueId()),
				"_command": "fiftyone",
				"_args":    []string{"delegated", "launch"},
			}
			actualJobYaml, err := convertJinjaToYAML(configMap.Data["testJob.yaml"], jinjaArgs)
			s.NoError(err)

			var job batchv1.Job
			helm.UnmarshalK8SYaml(subT, actualJobYaml, &job)

			testCase.expected(subT, job)
		})
	}
}

func envContains(envs []corev1.EnvVar, name, value string) bool {
	for _, e := range envs {
...
func volumeMountContains(vms []corev1.VolumeMount, name, mountPath string) bool {
	for _, vm := range vms {
...
func volumeContains(vols []corev1.Volume, name string) bool {
	for _, v := range vols {
		if v.Name == name {
			return true
		}
