...
func (s *deploymentAppTemplateTest) TestShareProcessNamespace() {
	testCases := []struct {
		name              string
		values            map[string]string
...
func (s *deploymentAppTemplateTest) TestSidecarContainerImage() {
	cInfo, err := chartInfo(s.T(), s.chartPath)
	s.NoError(err)
	chartAppVersion, exists := cInfo["appVersion"]
	s.True(exists, "failed to get app version from chart info")
...
func (s *deploymentAppTemplateTest) TestSidecarContainerImagePullPolicy() {
	testCases := []struct {
		name     string
		values   map[string]string
...
func (s *deploymentAppTemplateTest) TestSidecarContainerResourceRequirements() {
	testCases := []struct {
		name     string
		values   map[string]string
		expected func(resourceRequirements corev1.ResourceRequirements)
...
// capabilities and disables privilege escalation. The app deployment is
...
func (s *deploymentAppTemplateTest) TestSidecarContainerSecurityContext() {
